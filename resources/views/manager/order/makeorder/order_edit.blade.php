    ...
    <div class="row">
            ...
            <div class="bg-light user-select-none" style="height: 468px; overflow-y:scroll;">
                @foreach($menus as $menu)
                    <div class="record p-1 rounded-2 my-1 order" style="display: inline-block; z-index: 99; position: relative; background-color: rgb(246, 246, 246); border: 1px solid rgb(221, 221, 221)" onclick="handleOrder(event)" data-key={{$menu->id}} role="button">
                        ...
                        <div class="" onclick="handleOrder(event)" style="z-index: -1; position: relative;">
        ...
        <div class="col-lg-5 px-3">
            <div class="col-12 pt-1 pe-1">
                <form action="/waiter/order/update/{{$order->id}}" method="POST">
                    @csrf
                    @method('PUT')
                 ...
                 <div class="table-responsive"  style="height: 280px; overflow-y:scroll;">
                    <table class="table order-list table-bordered mt-3 pe-1" style="width: 100%; font-size:13px" class="my-3">
                        ...
                        <input id="hidden-items" class="d-none" name="hidden-items">
                
                    </table>
                    <div class="text-center cart opacity-25 mt-5 user-select-none"><img src="/storage/logo/cart1.png" width="60px" height="60px"/><p class="mt-1">Cart Empty</p></div>
                ...
                <div class="d-flex">
                    <div class="col-4">
                        <div class="p-2 rounded-1 px-2 me-1" class="bg-secondary" style="background-color:rgb(236, 236, 236); font-size: 14px"><span>Subtotal:</span>
                            <span>$ <input type="number" step="0.01" id="total" name="total" readonly value="0" style="outline:none; border:none; width: 50%; background:transparent"></span>
                        </div>
                    </div>
                    <div class="col-4">
        ...
        records.each((index, record) => {
            if (value == 'All Menus') {
                ...
                $(record).toggle(categoryName == value);
                        ...
                        totalItems.push({
                            'id': item.id,
                            'quantity': item.pivot.unit_quantity,
                            'subtotal': item.pivot.subtotal,
                            'status' : item.pivot.menu_status,
                        });
                        }
                        if(totalItems){
                        ...
                        for(let item of orderedItems){
                            $('#item-list').append(
                                        `<tr id='item-row' data-key=${key}>
                                            ...
                                            <td>${item.menuname}</td>
                                            <td class='rprice'  id='price'>${item.price}</td>
                                            <td><input type="number" class='rquantity' style="border:none; width:60%; outline:none" data-key=${key} value='${item.pivot.unit_quantity}' min=0 max=${item.pivot.unit_quantity > item.quantity ? item.pivot.unit_quantity : item.quantity}></td>
                                            <td id='subtotal'>${item.pivot.subtotal}</td>
                    ...
                    $('#grand-total').val(grandTotal);
                        ...
                        function updateQuantity() {
                            let row = event.target.closest('tr');
                                    ...
                                    subtotal = 0;
                                    subtotal = parseFloat(rprice) * quantity;
                                    rsubtotal.text(subtotal.toFixed(2));
